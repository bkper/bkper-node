...
export async function handleGetGroups(params: GetGroupsParams): Promise<CallToolResult> {
  try {
    ...
    throw new McpError(
      ...
      `Failed to get groups: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}

export const getGroupsToolDefinition = {
  name: 'get_groups',
  description: 'Get all groups in a book in hierarchical JSON structure, showing parent-child relationships',
  inputSchema: {
    type: 'object',
    properties: {
      bookId: {
